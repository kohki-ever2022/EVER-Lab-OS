// src/i18n/locales/project.ts
export const project = {
  projects: { ja: 'プロジェクト', en: 'Projects' },
  newProject: { ja: '新規プロジェクト', en: 'New Project' },
  noProjects: { ja: 'プロジェクトがありません。', en: 'No projects yet.' },
  createFirstProject: {
    ja: '最初のプロジェクトを作成',
    en: 'Create your first project',
  },
  editProject: { ja: 'プロジェクト編集', en: 'Edit Project' },
  projectName: { ja: 'プロジェクト名', en: 'Project Name' },
  members: { ja: 'メンバー', en: 'Members' },
  taskManagement: { ja: 'タスク管理', en: 'Task Management' },
  newTask: { ja: '新規タスク', en: 'New Task' },
  taskStatusToDo: { ja: '未着手', en: 'To Do' },
  taskStatusInProgress: { ja: '進行中', en: 'In Progress' },
  taskStatusInReview: { ja: 'レビュー中', en: 'In Review' },
  taskStatusDone: { ja: '完了', en: 'Done' },
  editTask: { ja: 'タスク編集', en: 'Edit Task' },
  taskTitle: { ja: 'タスクのタイトル', en: 'Task title' },
  assignees: { ja: '担当者', en: 'Assignees' },
  projectLabel: { ja: 'プロジェクト', en: 'Project' },
  priority: { ja: '優先度', en: 'Priority' },
  taskDueDate: { ja: '期日', en: 'Due Date' },
  taskSaved: { ja: 'タスクを保存しました。', en: 'Task saved.' },
  taskSaveFailed: {
    ja: 'タスクの保存に失敗しました。',
    en: 'Failed to save task.',
  },
  deleteTaskConfirm: {
    ja: 'このタスクを削除しますか？',
    en: 'Are you sure you want to delete this task?',
  },
  taskDeleted: { ja: 'タスクを削除しました。', en: 'Task deleted.' },
  deleteTaskFailed: {
    ja: 'タスクの削除に失敗しました。',
    en: 'Failed to delete task.',
  },
  deleteEntryConfirm: {
    ja: 'このエントリーを削除しますか？',
    en: 'Are you sure you want to delete this entry?',
  },
  entryDeleted: { ja: 'エントリーを削除しました。', en: 'Entry deleted.' },
  entryTitle: { ja: 'エントリータイトル', en: 'Entry Title' },
  experimentDate: { ja: '実験日', en: 'Experiment Date' },
  relatedProject: { ja: '関連プロジェクト', en: 'Related Project' },
  tagsCommaSeparated: {
    ja: 'タグ (カンマ区切り)',
    en: 'Tags (comma-separated)',
  },
  contentMarkdown: {
    ja: '内容 (Markdown対応)',
    en: 'Content (Markdown supported)',
  },
  attachments: { ja: '添付ファイル', en: 'Attachments' },
  fileAttachmentWip: {
    ja: 'ファイル添付機能は開発中です。',
    en: 'File attachment feature is under development.',
  },
  electronicLabNotebook: {
    ja: '電子実験ノート',
    en: 'Electronic Lab Notebook',
  },
  newEntry: { ja: '新規エントリー', en: 'New Entry' },
  selectOrCreateEntry: {
    ja: 'エントリーを選択または新規作成してください。',
    en: 'Please select an entry or create a new one.',
  },
  projectProgressDashboard: {
    ja: 'プロジェクト進捗ダッシュボード',
    en: 'Project Progress Dashboard',
  },
  progress: { ja: '進捗', en: 'Progress' },
  totalTasks: { ja: '全タスク', en: 'Total Tasks' },
  projectTaskOverdue: { ja: '遅延', en: 'Overdue' },
  milestones: { ja: 'マイルストーン', en: 'Milestones' },
  totalEquipmentUsage: { ja: '総機器利用時間', en: 'Total Equipment Usage' },
  noProjectsToDisplay: {
    ja: '表示するプロジェクトがありません。',
    en: 'No projects to display.',
  },
  co2Incubator: { ja: 'CO2インキュベーター', en: 'CO2 Incubator' },
  ganttChart: { ja: 'ガントチャート', en: 'Gantt Chart' },
  myProjects: { ja: 'マイプロジェクト', en: 'My Projects' },
  project: { ja: 'プロジェクト', en: 'Project' },
};
