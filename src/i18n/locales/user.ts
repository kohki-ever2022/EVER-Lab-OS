// src/i18n/locales/user.ts
export const user = {
  signInToAccount: {
    ja: 'アカウントにサインイン',
    en: 'Sign in to your account',
  },
  emailAddress: { ja: 'メールアドレス', en: 'Email address' },
  password: { ja: 'パスワード', en: 'Password' },
  signIn: { ja: 'サインイン', en: 'Sign in' },
  signingIn: { ja: 'サインイン中...', en: 'Signing in...' },
  loginErrorUserNotFound: {
    ja: 'ユーザー情報の取得に失敗しました。',
    en: 'Failed to retrieve user data.',
  },
  loginErrorSessionFailed: {
    ja: 'ログインセッションの開始に失敗しました。',
    en: 'Failed to start login session.',
  },
  loginErrorInvalidCredentials: {
    ja: 'メールアドレスまたはパスワードが正しくありません。',
    en: 'Invalid email address or password.',
  },
  loginErrorGeneral: {
    ja: 'ログイン処理中にエラーが発生しました。',
    en: 'An error occurred during login.',
  },
  memberManagement: { ja: 'メンバー管理', en: 'Member Management' },
  addNewMember: { ja: '新規メンバー追加', en: 'Add New Member' },
  profile: { ja: 'プロフィール', en: 'Profile' },
  nameMinLength: {
    ja: '名前は2文字以上で入力してください。',
    en: 'Name must be at least 2 characters.',
  },
  permissionDeniedDeleteUser: {
    ja: 'このユーザーを削除する権限がありません。',
    en: 'Permission denied to delete this user.',
  },
  userSaved: {
    ja: 'ユーザー情報が保存されました。',
    en: 'User information saved.',
  },
  deleteUserConfirm: {
    ja: 'このユーザーを削除しますか？関連データも削除される可能性があります。',
    en: 'Are you sure you want to delete this user? Associated data may also be removed.',
  },
  userDeleted: { ja: 'ユーザーを削除しました。', en: 'User deleted.' },
  editUser: { ja: 'ユーザー情報編集', en: 'Edit User' },
  selectCompany: { ja: '会社を選択', en: 'Select Company' },
  company: { ja: '会社', en: 'Company' },
  role: { ja: '役割', en: 'Role' },
  statusUpdated: { ja: 'ステータスを更新しました。', en: 'Status updated.' },
  statusUpdateFailed: {
    ja: 'ステータスの更新に失敗しました。',
    en: 'Failed to update status.',
  },
  updateStatus: { ja: 'ステータスを更新', en: 'Update Status' },
  statusAvailable: { ja: '対応可能', en: 'Available' },
  statusBusy: { ja: '取り込み中', en: 'Busy' },
  statusAway: { ja: '離席中', en: 'Away' },
  statusMessage: { ja: 'ステータスメッセージ', en: 'Status Message' },
  dashboard: { ja: 'ダッシュボード', en: 'Dashboard' },
  welcome: { ja: 'ようこそ、', en: 'Welcome, ' },
  upcomingReservations: { ja: '今後の予約', en: 'Upcoming Reservations' },
  noUpcomingReservations: {
    ja: '今後の予約はありません。',
    en: 'No upcoming reservations.',
  },
  quickActions: { ja: 'クイックアクション', en: 'Quick Actions' },
  bookEquipment: { ja: '機器を予約する', en: 'Book Equipment' },
  viewProjects: { ja: 'プロジェクトを見る', en: 'View Projects' },
  consumablesStore: { ja: '消耗品ストア', en: 'Consumables Store' },
  announcements: { ja: 'お知らせ', en: 'Announcements' },
  noAnnouncements: { ja: 'お知らせはありません。', en: 'No announcements.' },
  favoriteEquipment: { ja: 'お気に入り機器', en: 'Favorite Equipment' },
  noFavoriteEquipment: {
    ja: 'お気に入りの機器はありません。',
    en: 'No favorite equipment.',
  },
};
