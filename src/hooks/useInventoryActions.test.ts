...
const mockUsers: User[] = [
  {
    ...
    roleCategory: RoleCategory.Facility,
  },
];
...
describe('useInventoryActions', () => {
  ...
  it('should update a consumable successfully', async () => {
    ...
    (mockAdapter.updateConsumable as any).mockResolvedValue({
      success: true,
    ...
    expect(mockAdapter.updateConsumable).toHaveBeenCalledWith(
      updatedConsumable
    );
  });
  ...
  it('should add a consumable successfully', async () => {
    const newConsumable = {
      ...
      stock: 50,
      lowStockThreshold: 10,
    } as Omit<Consumable, 'id'>;
    (mockAdapter.createConsumable as any).mockResolvedValue({
      success: true,
      data: { ...newConsumable, id: 'c4' },
    });
    const { result } = renderHook(() => useInventoryActions(), { wrapper });
  ...
  it('should successfully create an order and log audit', async () => {
    ...
    (mockAdapter.createOrder as any).mockResolvedValue({
      success: true,
      data: expectedOrder,
    });
    const { result } = renderHook(() => useInventoryActions(), { wrapper });
    ...
    expect(mockAddAuditLog).toHaveBeenCalledWith(
      'ORDER_CREATE',
      expect.stringContaining('ordered 2 of consumable c1')
    );
