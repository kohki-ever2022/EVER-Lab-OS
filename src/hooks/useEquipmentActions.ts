...
export const useEquipmentActions = () => {
  ...
  const addEquipment = useCallback(
    async (
      equipmentItem: Omit<Equipment, 'id'>
    ): Promise<Result<Equipment, Error>> => {
      const result = await adapter.createEquipment(equipmentItem);
      if (result.success) {
        addAuditLog(
          'EQUIPMENT_CREATE',
          `Created equipment '${result.data.nameEN}'`
        );
      }
      return result;
    },
  ...
  const updateEquipment = useCallback(
    async (equipmentItem: Equipment): Promise<Result<Equipment, Error>> => {
      ...
      if (result.success) {
        addAuditLog(
          'EQUIPMENT_UPDATE',
          `Updated equipment '${equipmentItem.nameEN}'`
        );

        if (
          prevEq &&
          prevEq.status !== EquipmentStatus.Maintenance &&
          equipmentItem.status === EquipmentStatus.Maintenance
        ) {
          const staffToNotify = users.filter(
            (u) => u.roleCategory === RoleCategory.Facility
          );
          const equipmentName = isJapanese
            ? equipmentItem.nameJP
            : equipmentItem.nameEN;
          const notificationMessage = t('notificationEquipmentMaintenance', {
            name: equipmentName,
          });
  ...
  const deleteEquipment = useCallback(
    async (equipmentId: string): Promise<Result<void, Error>> => {
      ...
      if (result.success) {
        addAuditLog(
          'EQUIPMENT_DELETE',
          `Deleted equipment '${itemToDelete.nameEN}'`
        );
      }
      return result;
    },
  ...
  const updateCO2IncubatorTracking = useCallback(
    (data: CO2IncubatorTracking): Result<CO2IncubatorTracking, Error> => {
      try {
        setCo2IncubatorTrackingData((prev) =>
  ...
  const emergencyStopAllEquipment = useCallback(async (): Promise<
    Result<void, Error>
  > => {
    try {
      const equipmentToStop = equipment.filter(
        (e) => e.status !== EquipmentStatus.Maintenance
      );
      const updates = equipmentToStop.map((e) =>
        adapter.updateEquipment({ ...e, status: EquipmentStatus.Maintenance })
      );
