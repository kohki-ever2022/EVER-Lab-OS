...
import {
  canAccessData as checkDataScope,
  canAccessAllTenants as checkAllTenantsAccess,
  filterByTenantAccess as filterData,
  ...
  const permissionsData = useMemo(() => {
    if (!currentUser) {
      return {
        ...
        canAccessData: (dataOwnerId: string, dataTenantId: string): boolean =>
          false,
    ...
    const permissions =
      ROLE_PERMISSIONS.find((rp) => rp.role === currentUser.role)
        ?.permissions || [];
    ...
    const filterByTenantAccess = <
      T extends { userId?: string; companyId?: string },
    >(
      data: T[]
    ): T[] => {
    ...
    const canManageMFSettings = (): boolean =>
      hasPermission('moneyforward', 'manage');
