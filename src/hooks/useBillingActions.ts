...
export const useBillingActions = () => {
    ...
    const addInvoice = useCallback(async (invoiceItem: Omit<Invoice, 'id'>): Promise<Result<Invoice, Error>> => {
        return await adapter.createInvoice(invoiceItem);
    }, [adapter]);
  
    const updateInvoice = useCallback(async (invoiceItem: Invoice): Promise<Result<Invoice, Error>> => {
        return await adapter.updateInvoice(invoiceItem);
    }, [adapter]);

    const calculateCostForUsage = useCallback((usage: Usage, eq: Equipment): number => {
        if (!eq) return 0;
        ...
        if (eq.rateUnitEN.toLowerCase().includes('cycle')) {
            ...
            const granularityMatch = eq.granularity.match(/(\d+)分単位・(切上げ|四捨入)/);
        ...
        if (cost > 0 && systemSettings.surgePricingEnabled) {
            ...
            const surgeStartHour = parseInt(systemSettings.surgeStartTime.split(':')[0]);
            const surgeEndHour = parseInt(systemSettings.surgeEndTime.split(':')[0]);
