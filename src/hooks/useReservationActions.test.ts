...
const mockCurrentUser: User = {
  ...
  roleCategory: RoleCategory.Tenant,
};
...
describe('useReservationActions', () => {
  ...
  it('should add a reservation successfully', async () => {
    ...
    (mockAdapter.createReservation as any).mockResolvedValue({
      success: true,
      data: { ...newReservation, id: 'res-2' },
    });
  ...
  it('should update a reservation successfully', async () => {
    ...
    (mockAdapter.updateReservation as any).mockResolvedValue({
      success: true,
    ...
    await act(async () => {
      actionResult =
        await result.current.updateReservation(reservationToUpdate);
    });

    expect(mockAdapter.updateReservation).toHaveBeenCalledWith(
      reservationToUpdate
    );
  ...
  it('should check out a reservation and create a usage record', async () => {
    ...
    (mockAdapter.createUsage as any).mockResolvedValue({
      success: true,
      data: { id: 'usage-1', ...expectedUsage },
    });
    ...
    let actionResult;
    await act(async () => {
      actionResult = await result.current.checkOutReservation(
        reservationToCheckOut.id
      );
    });
    ...
    expect(mockAdapter.createUsage).toHaveBeenCalledWith(
  ...
  it('should add an item to the waitlist', async () => {
    ...
    let actionResult;
    await act(async () => {
      actionResult = await result.current.addToWaitlist(
        'eq-1',
        new Date(),
        new Date()
      );
    ...
    expect(actionResult?.success).toBe(true);
    expect((actionResult as any).data).toEqual(
      expect.objectContaining({
        equipmentId: 'eq-1',
