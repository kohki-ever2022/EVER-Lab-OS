...
export const useInventoryActions = () => {
  ...
  const updateConsumable = useCallback(
    async (consumable: Consumable): Promise<Result<Consumable, Error>> => {
      try {
        ...
        const result = await adapter.updateConsumable(
          sanitizeObject(consumable)
        );

        if (result.success && prevConsumable && addNotification && users) {
          const isNowLow =
            consumable.stock > 0 &&
            consumable.stock <= consumable.lowStockThreshold;
          const wasPreviouslyOk =
            prevConsumable.stock > prevConsumable.lowStockThreshold;
          if (isNowLow && wasPreviouslyOk) {
            const staffToNotify = users.filter(
              (u) =>
                u.role === Role.LabManager || u.role === Role.FacilityDirector
            );
  ...
  const addConsumable = useCallback(
    ...
    ): Promise<Result<Consumable, Error>> => {
      try {
  ...
  const deleteConsumable = useCallback(
    async (consumableId: string): Promise<Result<void, Error>> => {
      try {
  ...
  const addOrder = useCallback(
    async (
      order: Omit<Order, 'id' | 'totalPrice' | 'orderDate' | 'status'>
      ...
      if (consumable.stock < order.quantity)
      ...
      if (result.success) {
        addAuditLog(
          'ORDER_CREATE',
          `User ${order.userId} ordered ${order.quantity} of consumable ${order.consumableId}.`
        );
      }
      return result;
    },
