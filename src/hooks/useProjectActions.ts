...
export const useProjectActions = () => {
  ...
  const addProject = useCallback(
    async (project: Omit<Project, 'id'>): Promise<Result<Project, Error>> => {
  ...
  const updateProject = useCallback(
    async (project: Project): Promise<Result<Project, Error>> => {
      const sanitizedProject = sanitizeObject(project);
  ...
  const addLabNotebookEntry = useCallback(
    async (
      entry: Omit<LabNotebookEntry, 'id' | 'userId' | 'createdAt' | 'updatedAt'>
    ): Promise<Result<LabNotebookEntry, Error>> => {
      ...
      if (result.success) {
        addAuditLog(
          'ELN_ENTRY_CREATE',
          `Created ELN entry '${result.data.title}'`
        );
      }
      return result;
    },
  ...
  const updateLabNotebookEntry = useCallback(
    async (
      ...
      const result = await adapter.updateLabNotebookEntry(
        updatedEntry as LabNotebookEntry
      );
      if (result.success) {
  ...
  const deleteLabNotebookEntry = useCallback(
    async (entryId: string): Promise<Result<void, Error>> => {
      ...
      if (result.success && entryToDelete) {
        addAuditLog(
          'ELN_ENTRY_DELETE',
          `Deleted ELN entry '${entryToDelete.title}'`
        );
      }
      return result;
    },
  ...
  const addTask = useCallback(
    async (
      task: Omit<Task, 'id' | 'createdByUserId' | 'createdAt' | 'updatedAt'>
  ...
  const updateTask = useCallback(
    async (task: Task): Promise<Result<Task, Error>> => {
  ...
  const deleteTask = useCallback(
    async (taskId: string): Promise<Result<void, Error>> => {
