...
const ElectronicLabNotebook: React.FC = () => {
  ...
  const filteredEntries = useMemo(() => {
    return (labNotebookEntries || [])
      ...
      .filter((entry) => {
        if (dateFilter) {
          return new Date(entry.experimentDate)
            .toISOString()
      ...
      .sort(
        (a, b) =>
          new Date(b.experimentDate).getTime() -
          new Date(a.experimentDate).getTime()
      );
  ...
  const selectedEntry = useMemo(() => {
    ...
    return (
      (labNotebookEntries || []).find((e) => e.id === selectedEntryId) || null
    );
  ...
  const EntryForm: React.FC<{ entry: Partial<LabNotebookEntry> | null }> = ({
    entry,
  }) => {
    const [title, setTitle] = useState(entry?.title || '');
    const [experimentDate, setExperimentDate] = useState(
      entry?.experimentDate
        ? new Date(entry.experimentDate).toISOString().split('T')[0]
    ...
    return (
      <div className='p-4 sm:p-6 space-y-4'>
        <input
          ...
          className='w-full text-2xl font-bold border-b-2 pb-2 focus:outline-none focus:border-ever-blue'
        />
        ...
        <div>
          ...
          <textarea
            ...
            rows={15}
            className='mt-1 w-full border rounded-md p-2 font-mono text-sm'
          ></textarea>
        </div>
        <div>
          <label className='block text-sm font-medium text-gray-500'>
        ...
        <div className='flex justify-end gap-2'>
          ...
          <button
            onClick={handleSave}
            className='px-4 py-2 bg-ever-blue text-white rounded-md'
          >
            {t('save')}
          </button>
        </div>
  ...
  const EntryViewer: React.FC<{ entry: LabNotebookEntry }> = ({ entry }) => {
    const project = entry.projectId
      ? myProjects.find((p) => p.id === entry.projectId)
      : null;
    return (
        ...
        <div className='flex justify-between items-start mb-4'>
          <div>
            ...
            <p className='text-sm text-gray-500'>
              {t('experimentDate')}:{' '}
              {new Date(entry.experimentDate).toLocaleDateString()}
            </p>
            {project && (
              <p className='text-sm text-gray-500'>
        ...
        <div className='w-full md:w-1/3 bg-white rounded-lg shadow-md flex flex-col'>
          <div className='p-4 border-b'>
            ...
            <button
              ...
              className='w-full bg-ever-blue text-white font-bold py-2 px-4 rounded-lg text-sm'
            >
              {t('newEntry')}
            </button>
          </div>
          <div className='overflow-y-auto'>
            {filteredEntries.map((entry) => (
              <div
                ...
                className={`p-4 border-b cursor-pointer hover:bg-gray-50 ${selectedEntryId === entry.id ? 'bg-ever-blue-light' : ''}`}
              >
