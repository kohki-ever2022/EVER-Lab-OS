...
const DashboardCard: React.FC<{
  ...
  onViewAll?: () => void;
  viewAllText?: string;
}> = ({ title, icon, children, onViewAll, viewAllText }) => (
  <div className='bg-white p-6 rounded-lg shadow flex flex-col'>
    <div className='flex justify-between items-center mb-4'>
      ...
      {onViewAll && (
        <button
          onClick={onViewAll}
          className='text-sm font-semibold text-ever-blue hover:text-ever-blue-dark'
        >
          {viewAllText}
        </button>
      )}
          ...
          >
            <div className='space-y-3'>
              <button
                ...
                className='w-full text-left p-3 bg-ever-blue-light hover:bg-ever-blue-light/80 rounded-md font-semibold text-ever-blue-dark transition-colors'
              >
                {t('bookEquipment')}
              </button>
              <button
                onClick={() => handleNavigate('projects')}
                className='w-full text-left p-3 bg-ever-purple-light hover:bg-ever-purple-light/80 rounded-md font-semibold text-ever-purple-dark transition-colors'
              >
                {t('viewProjects')}
              </button>
              <button
                onClick={() => handleNavigate('consumablesStore')}
                className='w-full text-left p-3 bg-green-100 hover:bg-green-200/80 rounded-md font-semibold text-green-800 transition-colors'
              >
                {t('consumablesStore')}
              </button>
            </div>
              ...
              <ul className='space-y-3'>
                {latestAnnouncements.map((a) => (
                  <li key={a.id} className='p-3 bg-gray-50 rounded-md'>
                    <p className='font-semibold text-gray-800'>
                      {isJapanese ? a.titleJP : a.titleEN}
                    </p>
              ...
              <ul className='space-y-3'>
                {favoriteEquipment.map((eq) => (
                  <li
                    ...
                    onClick={() => handleFavoriteClick(eq)}
                  >
                    <div>
                      <p className='font-semibold text-gray-800'>
                        {isJapanese ? eq.nameJP : eq.nameEN}
