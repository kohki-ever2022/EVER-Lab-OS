          ...
          <div key={project.id} className="bg-white rounded-lg shadow p-6 flex flex-col">
            ...
            <div className="grid grid-cols-3 gap-2 mb-4 text-center">
              <div className="bg-gray-50 p-2 rounded">
                <div className="text-xs text-gray-600">{isJapanese ? '総タスク' : 'Total'}</div>
                <div className="text-lg font-bold">{project.totalTasks}</div>
              </div>
              <div className="bg-green-50 p-2 rounded">
                <div className="text-xs text-green-600">{isJapanese ? '完了' : 'Done'}</div>
                <div className="text-lg font-bold text-green-700">{project.completedTasks}</div>
              </div>
              <div className="bg-red-50 p-2 rounded">
                <div className="text-xs text-red-600">{isJapanese ? '遅延' : 'Overdue'}</div>
                <div className="text-lg font-bold text-red-700">{project.overdueTasks}</div>
              </div>
            </div>

            {/* Milestones */}
            {project.milestones.length > 0 && <div className="mb-4">
              <div className="text-sm font-medium mb-2">
                {isJapanese ? 'マイルストーン' : 'Milestones'}
              </div>
              <div className="flex gap-2 flex-wrap">
                {project.milestones.map(m => {
                    const isOverdue = m.status === MilestoneStatus.Pending && new Date(m.dueDate) < new Date();
                    const milestoneClass = 
                      m.status === MilestoneStatus.Completed ? 'bg-green-100 text-green-800' :
                      m.status === MilestoneStatus.InProgress ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="text-sm text-gray-600 mt-auto pt-4 border-t">
              {isJapanese ? '総機器使用時間' : 'Total Equipment Usage'}: 
              <span className="font-bold ml-2">{project.totalUsageHours}h</span>
            </div>
          </div>
        ))}
        {projectStats.length === 0 && <p className="text-gray-500 col-span-full text-center py-8">{isJapanese ? '表示するプロジェクトがありません。' : 'No projects to display.'}</p>}
      </div>
    </div>
  );
};
