...
const HazardousMaterialsDashboard: React.FC = () => {
  ...
  const totalMultiple = hazardousConsumables.reduce(
  ...
  type CompanyItemsMap = Record<
    string,
    { items: (Consumable & { multiple: number })[]; totalMultiple: number }
  >;

  const itemsByCompany = hazardousConsumables.reduce(
    (acc: CompanyItemsMap, item) => {
          ...
          return (
            <div key={companyId} className='bg-white p-6 rounded-lg shadow'>
              <h3 className='text-xl font-semibold mb-4'>
                {getCompanyName(companyId)} - {t('totalMultiple')}:{' '}
                {data.totalMultiple.toFixed(3)}
              </h3>
                    ...
                    {data.items.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>
                          {item.hazardousCategory}
                        </td>
                        <td className='px-6 py-4 whitespace-nowrap'>
                          {item.stock * (item.packageSize || 0)}{' '}
                          {item.packageUnit}
                        </td>
                        <td className='px-6 py-4 whitespace-nowrap font-mono'>
                          {item.multiple.toFixed(3)}
                        </td>
                        <td className='px-6 py-4 whitespace-nowrap text-right text-sm font-medium'>
                          <button
                            onClick={() => handleViewSds(item.sdsId)}
