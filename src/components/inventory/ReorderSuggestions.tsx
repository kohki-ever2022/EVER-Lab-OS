          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {stockoutItems.map((item) => (
              <div key={item.id} className="bg-red-50 border border-red-200 p-4 rounded-lg">
                ...
                <p className="text-sm text-gray-600">{t('category')}: {isJapanese ? item.categoryJP : item.categoryEN}</p>
                <p className="text-sm text-red-700 font-bold">{t('currentStock')}: {item.stock}</p>
              </div>
            ))}
          </div>
        ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {suggestedItems.map((item) => (
              <div key={item.id} className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                <p className="font-bold">{isJapanese ? item.nameJP : item.nameEN}</p>
                <p className="text-sm text-gray-600">{t('category')}: {isJapanese ? item.categoryJP : item.categoryEN}</p>
                <p className="text-sm text-yellow-700 font-bold">{t('currentStock')}: {item.stock}</p>
                <p className="text-sm text-gray-500">{t('threshold')}: {item.lowStockThreshold}</p>
              </div>
            ))}
          </div>
        ) : (
