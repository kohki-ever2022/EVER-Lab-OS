...
const SupplierDashboard: React.FC = () => {
    ...
    const renderQuotes = (quotes: Quotation[]) => {
        if (quotes.length === 0) {
            return <p className="text-gray-500 py-8 text-center">{t('noRequestsFound')}</p>
        }
        return (
                ...
                {quotes.map(q => {
                    ...
                    return (
                        <div key={q.id} className="bg-white p-4 rounded-lg shadow-sm border">
                            ...
                            <p className="text-sm mt-2">{t('quantity')} {q.quantity}</p>
                            <p className="text-xs text-gray-500 mt-1">{t('requestedOn')} {new Date(q.requestDate).toLocaleDateString()}</p>
                            
                            {activeTab === 'PENDING' && !myResponse && (
                                <div className="mt-4 pt-4 border-t space-y-3">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="text-xs font-medium">{t('priceYen')}</label>
                                        ...
                                        <div>
                                            <label className="text-xs font-medium">{t('deliveryDate')}</label>
                            ...
                            {myResponse && (
                                <div className="mt-3 pt-3 border-t bg-gray-50 p-3 rounded">
                                    ...
                                    <p className="text-sm">{t('price')}: ¥{myResponse.price?.toLocaleString()}</p>
                                    <p className="text-sm">{t('deliveryDate')}: {myResponse.deliveryDate ? new Date(myResponse.deliveryDate).toLocaleDateString() : 'N/A'}</p>
                                    {myResponse.notes && <p className="text-sm mt-1 text-gray-600">{t('notes')}: {myResponse.notes}</p>}
                                </div>
                            )}
                        </div>
                    );
