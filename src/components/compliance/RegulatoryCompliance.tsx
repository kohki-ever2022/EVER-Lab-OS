...
const RegulatoryCompliance: React.FC = () => {
  ...
  const [selectedType, setSelectedType] = useState<RegulationType | 'ALL'>(
    'ALL'
  );
  ...
  const canManageSupport =
    isFacilityStaff || hasPermission('regulatory', 'manage');
  ...
  const getDeadlineWarning = (deadline?: Date) => {
    ...
    if (daysUntilDeadline < 0) {
      ...
      return {
        text: `${t('daysLeft')}${daysUntilDeadline}${t('daysLeftSuffix')}`,
  ...
  return (
    <div className='p-6'>
      <div className='flex justify-between items-center mb-6'>
        <h2 className='text-2xl font-bold'>{t('regComplianceManagement')}</h2>
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-4 mb-6'>
          ...
          <div className='text-2xl font-bold text-red-700'>
            {
              filteredRequirements.filter(
                (r) => r.submissionStatus === SubmissionStatus.Required
              ).length
            }
          </div>
        </div>
        <div className='bg-yellow-50 p-4 rounded-lg border border-yellow-200'>
          ...
          <div className='text-2xl font-bold text-yellow-700'>
            {
              filteredRequirements.filter(
                (r) => r.submissionStatus === SubmissionStatus.Preparing
              ).length
            }
          </div>
        </div>
        <div className='bg-blue-50 p-4 rounded-lg border border-blue-200'>
          ...
          <div className='text-2xl font-bold text-blue-700'>
            {
              filteredRequirements.filter(
                (r) => r.submissionStatus === SubmissionStatus.Submitted
              ).length
            }
          </div>
        </div>
        <div className='bg-green-50 p-4 rounded-lg border border-green-200'>
          ...
          <div className='text-2xl font-bold text-green-700'>
            {
              filteredRequirements.filter(
                (r) => r.submissionStatus === SubmissionStatus.Approved
              ).length
            }
          </div>
        </div>
      ...
      <div className='bg-white p-4 rounded-lg shadow mb-6'>
        <label className='block text-sm font-medium mb-2'>
          {t('filterByRegulation')}
        </label>
        <select
          value={selectedType}
          onChange={(e) =>
            setSelectedType(e.target.value as RegulationType | 'ALL')
          }
              ...
              return (
                <tr key={req.id} className='hover:bg-gray-50'>
                  <td className='px-6 py-4 whitespace-nowrap text-sm'>
                    {t(regulationTypeLabels[req.type].key as any)}
                  </td>
                  <td className='px-6 py-4 text-sm'>
                    <div className='font-medium'>
                      {isJapanese
                        ? req.requirementNameJP
                        : req.requirementNameEN}
                    </div>
                  ...
                  <td className='px-6 py-4 whitespace-nowrap text-sm'>
                    {req.submissionDeadline ? (
                      <>
                        ...
                        {deadlineWarning && (
                          <div
                            className={`text-xs mt-1 font-medium ${deadlineWarning.color}`}
                          >
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div className='bg-white p-4 rounded-lg'>
              <h4 className='font-bold text-blue-700 mb-3'>
                {t('socialInsuranceLaborConsultant')}
              </h4>
