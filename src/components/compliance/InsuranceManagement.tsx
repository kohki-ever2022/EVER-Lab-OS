...
const InsuranceManagement: React.FC = () => {
  ...
  const handleSubmitCertificate = (e: React.FormEvent) => {
      ...
      reader.onload = async () => {
        ...
        try {
            ...
            const insuranceTypeLabel = t(insuranceTypeLabels[newCertificateData.type].key as any);
            ...
            if (addResult.success === false) {
                showToast(`${t('certSaveFailed')}: ${addResult.error.message}`, 'error');
                return;
            }
  ...
  const handleCertificateRenewed = async (certificate: InsuranceCertificate, newCertificate: InsuranceCertificate) => {
    try {
      ...
      const insuranceTypeLabel = t(insuranceTypeLabels[newCertificate.type].key as any);
      const company = companies.find(c => c.id === newCertificate.tenantId);
      const companyName = company ? (isJapanese ? company.nameJP : company.nameEN) : t('tenant');
      ...
      if (updateResult.success === false) {
        showToast(`${t('certRenewFailed')}: ${updateResult.error.message}`, 'error');
        return;
      }
  ...
  const getExpiryStatus = (endDate: Date) => {
    ...
    if (daysUntilExpiry < 0) {
      ...
      return { status: 'warning', label: `${t('daysLeft')}${daysUntilExpiry}${t('daysLeftSuffix')}`, color: 'bg-yellow-100 text-yellow-700' };
    }
    return { status: 'valid', label: t('valid'), color: 'bg-green-100 text-green-700' };
  };
          ...
          <div className="bg-white p-6 rounded-lg shadow mb-6 border-2 border-blue-200">
             ...
             <form onSubmit={handleSubmitCertificate} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium">{t('insuranceType')}</label>
                    <select name="type" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value as InsuranceType})} className="w-full border rounded p-2 mt-1">
                      {Object.entries(insuranceTypeLabels).map(([key, {key: tKey}]) => <option key={key} value={key}>{t(tKey as any)}</option>)}
                    </select>
                  </div>
                   <div>
                    <label className="block text-sm font-medium">{t('insuranceCompany')}</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium">{t('policyNumber')}</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium">{t('coverageAmount')}</label>
                ...
                <div>
                  <label className="block text-sm font-medium">{t('certFile')}</label>
                  <input type="file" accept=".pdf" onChange={handleFileChange} className="w-full border rounded p-2 mt-1" required />
                </div>
                 <div className="flex justify-end gap-2 mt-4">
                   <button type="button" onClick={() => setShowUploadForm(false)} className="px-4 py-2 border rounded hover:bg-gray-50">{t('cancel')}</button>
                   <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">{t('upload')}</button>
                 </div>
             </form>
          </div>
      )}
      ...
      <div className="bg-white rounded-lg shadow overflow-x-auto">
        <table className="min-w-full">
            <thead className="bg-gray-50">
                <tr>
                    {isFacilityStaff && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('tenant')}</th>}
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('insuranceType')}</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('policyNumber')}</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('expiryDate')}</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
                {myCertificates.map(cert => {
                    ...
                    const status = cert.status === 'EXPIRED' ? expiry : { label: t(statusConfigEntry.key as any), color: statusConfigEntry.color };
                    return (
                        <tr key={cert.id} className="hover:bg-gray-50">
                            {isFacilityStaff && <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{getCompanyName(cert.tenantId)}</td>}
                            <td className="px-6 py-4 whitespace-nowrap text-sm">{t(insuranceTypeLabels[cert.type].key as any)}</td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                                <a href={cert.certificateUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 mr-3">{t('view')}</a>
                                {isFacilityStaff && cert.status === 'PENDING' && (
                                    <button onClick={() => handleVerify(cert)} className="text-green-600 hover:text-green-800">{t('verify')}</button>
                                )}
                            </td>
                        </tr>
                    )
