    ...
    <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4'>
      <div className='bg-white rounded-lg shadow-xl p-6 w-full max-w-lg'>
        ...
        <div className='mt-6 flex justify-end'>
          <button
            onClick={onClose}
            className='bg-lab-blue text-white font-bold py-2 px-4 rounded-lg'
          >
            {t('close')}
          </button>
        </div>
...
const AdminDashboard: React.FC = () => {
  ...
  const maintenanceCount = useMemo(
    () =>
      equipment.filter((e) => e.status === EquipmentStatus.Maintenance).length,
    [equipment]
  );
  ...
  const unreadMemos = useMemo(
    () =>
      memos.filter((m) => m.recipientId === currentUser?.id && !m.isRead)
        .length,
    [memos, currentUser]
  );
  ...
  const malfunctionReports = useMemo(
    () =>
      maintenanceLogs.filter(
        (log) => log.status === MaintenanceLogStatus.Reported
      ),
    [maintenanceLogs]
  );

  const activeReservations = useMemo(
    () => reservations.filter((r) => r.status === ReservationStatus.CheckedIn),
    [reservations]
  );
    ...
    if (window.confirm(t('emergencyStopConfirm'))) {
      if (emergencyStopAllEquipment) {
        ...
        if (result.success === false) {
          showToast(
            t('emergencyStopFailed') + `: ${result.error.message}`,
            'error'
          );
        } else {
  ...
  const usageRanking = useMemo(() => {
    ...
    const usageByEquipment = recentUsage.reduce(
    ...
    <div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8'>
        <InfoCard
          ...
          icon={<ClockIcon className='h-6 w-6 text-ever-blue' />}
        />
        <InfoCard
        ...
        <InfoCard
          title={t('unreadMemos')}
          value={unreadMemos}
          icon={<MailIcon className='h-6 w-6 text-ever-blue' />}
        />
        <InfoCard
          title={t('malfunctionReports')}
          value={malfunctionReports.length}
          icon={<ExclamationCircleIcon className='h-6 w-6 text-ever-blue' />}
        />
        <InfoCard
          title={t('inMaintenance')}
          value={maintenanceCount}
          icon={<CogIcon className='h-6 w-6 text-ever-blue' />}
        />
      </div>

      <div className='bg-white p-4 rounded-lg shadow mb-8'>
        <h3 className='text-xl font-bold text-ever-black mb-4'>
          {t('adminActions')}
        </h3>
        <div className='flex flex-wrap gap-4'>
          ...
          {currentUser?.role === Role.FacilityDirector && (
            <button
              ...
              className='bg-gradient-to-r from-red-600 to-red-700 text-white hover:from-red-500 hover:to-red-600 font-bold py-2 px-4 rounded-lg text-sm'
            >
              {t('emergencyStopAll')}
            </button>
          )}
