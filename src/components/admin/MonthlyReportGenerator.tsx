...
const MonthlyReportGenerator: React.FC = () => {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      const aggregatedData = aggregateMonthlyData(
        selectedYear,
        selectedMonth + 1,
        dataSources
      );
  ...
  return (
    <div>
      <h2 className='text-3xl font-bold mb-6 text-ever-black no-print'>
        {t('monthlyOperationsReport')}
      </h2>
              ...
              {monthlyReports.map((report) => (
                <li key={report.id}>
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-md transition-colors ${currentReportPeriod === report.period ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
                  >
            ...
            <div className='bg-white p-6 rounded-lg shadow-md'>
              <div className='flex justify-between items-center mb-4 border-b pb-4 no-print'>
                <h3 className='text-2xl font-bold'>
                  {t('reportForPeriod')}: {currentReportPeriod}
                </h3>
                <div className='flex gap-2'>
                  <button
                    onClick={handleDownloadPdf}
                    className='bg-white hover:bg-gray-100 text-gray-800 font-semibold py-1 px-3 border border-gray-300 rounded shadow text-sm'
                  >
                    {t('downloadPdf')}
                  </button>
                  <button
