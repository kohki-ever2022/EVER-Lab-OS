...
export const BackupManager: React.FC = () => {
  ...
  const handleRestore = async () => {
    if (!selectedBackup || !window.confirm(`${selectedBackup}のバックアップをリストアしますか？ この操作は現在のデータベースを上書きする可能性があり、元に戻せません。`)) return;

    setIsLoading(true);
    setStatusMessage('');
    try {
      const result = await restoreBackup({ backupDate: selectedBackup });
      setStatusMessage(`リストアが開始されました。Operation: ${(result.data as any).operation}`);
      alert('リストアが開始されました');
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="p-6 bg-white shadow-md rounded-lg">
      <h2 className="text-2xl font-bold mb-4 text-gray-800">データベースバックアップ管理</h2>
      ...
      {statusMessage && (
        <p className={`mt-4 text-sm ${statusMessage.includes('失敗') ? 'text-red-600' : 'text-green-600'}`}>
          {statusMessage}
        </p>
      )}
      <p className="mt-4 text-sm text-gray-600">注意: リストア処理はデータベース全体に影響します。実行には細心の注意を払ってください。</p>
    </div>
  );
};
