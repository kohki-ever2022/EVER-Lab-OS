...
const MaintenanceLogViewer: React.FC = () => {
  ...
  const logsWithDetails = useMemo(() => {
    return maintenanceLogs
      .map((log) => ({
        ...log,
        equipmentName:
          equipment.find((e) => e.id === log.equipmentId)?.[
            isJapanese ? 'nameJP' : 'nameEN'
          ] || 'N/A',
        reporterName:
          users.find((u) => u.id === log.reportedByUserId)?.name || 'N/A',
      }))
      .sort(
        (a, b) =>
  ...
  const getStatusColor = (status: MaintenanceLogStatus) => {
    switch (status) {
      case MaintenanceLogStatus.Reported:
        return 'bg-red-100 text-red-800';
      case MaintenanceLogStatus.InProgress:
