    ...
    return equipment
      ...
      .map((e) => {
        const latestLog = maintenanceLogs
          .filter(
            (log) =>
              log.equipmentId === e.id &&
              log.status !== MaintenanceLogStatus.Completed
          )
          .sort(
            (a, b) =>
        ...
        const reporter = latestLog
          ? users.find((u) => u.id === latestLog.reportedByUserId)
          : null;
      ...
      {maintenanceEquipment.length > 0 ? (
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {maintenanceEquipment.map((item) => (
            <div
              key={item.id}
              className='bg-white rounded-lg shadow border-l-4 border-yellow-500 p-6'
            >
              ...
              {item.latestLog ? (
                <div className='bg-yellow-50 p-4 rounded-md'>
                  ...
                  <p className='text-xs text-yellow-600 mt-2'>
                    {t('by')} {item.reporterName || 'N/A'} -{' '}
                    {new Date(item.latestLog.reportDate).toLocaleDateString()}
                  </p>
                </div>
              ) : (
