...
const StatusBadge: React.FC<{ status: EquipmentStatus }> = ({ status }) => {
  ...
  return (
    ...
    >
      <span
        className={`w-2 h-2 mr-1.5 rounded-full ${currentStatus.dot}`}
      ></span>
      {currentStatus.text}
    </span>
  );
...
const EquipmentCard: React.FC<{ equipment: EquipmentType }> = ({
  equipment,
}) => {
  ...
  const isThisFavorite = currentUser
    ? isFavorite(currentUser.id, equipment.id)
    : false;
  ...
  const requiredQual = requiredQualId
    ? qualifications.find((q) => q.id === requiredQualId)
    : null;

  const isCalibrationOverdue =
    equipment.nextCalibrationDate &&
    new Date(equipment.nextCalibrationDate) < new Date();

  const isAvailableForBooking =
    equipment.isReservable &&
    equipment.status === EquipmentStatus.Available &&
    userHasQualification &&
    !isCalibrationOverdue;

  const equipmentName = isJapanese ? equipment.nameJP : equipment.nameEN;
  const equipmentCategory = isJapanese
    ? equipment.categoryJP
    : equipment.categoryEN;
  const rateUnit = isJapanese ? equipment.rateUnitJP : equipment.rateUnitEN;
  const requiredQualName = requiredQual
    ? isJapanese
      ? requiredQual.nameJP
      : requiredQual.nameEN
    : '';
  const personInCharge = equipment.personInChargeUserId
    ? users.find((u) => u.id === equipment.personInChargeUserId)
    : null;
  const locationDetails = isJapanese
    ? equipment.locationDetailsJP
    : equipment.locationDetailsEN;
  ...
  const getBookingButtonTitle = () => {
    ...
    if (!userHasQualification) {
      if (!userCertification && requiredQualId) {
        const hasEverHadCert = userCertifications.some(
          (c) =>
            c.userId === currentUser?.id &&
            (c as any).qualificationId === requiredQualId
        );
  ...
  return (
    <div className='bg-white rounded-lg shadow-md overflow-hidden flex flex-col relative'>
      <button
        ...
        aria-label={
          isThisFavorite ? t('removeFromFavorites') : t('addToFavorites')
        }
      >
        <StarIcon
          className={`w-6 h-6 ${isThisFavorite ? 'fill-yellow-400 stroke-yellow-500' : 'fill-transparent stroke-white'}`}
        />
      </button>
      <img
      ...
      <div className='p-4 flex flex-col flex-grow'>
        ...
        <h3 className='text-lg font-bold text-ever-black mt-2'>
          {equipmentName}
        </h3>
        ...
        <div className='mt-2 text-sm text-gray-700 space-y-1'>
          <p>
            ...
            {equipment.isReservable
              ? `${equipment.rate.toLocaleString()} ${rateUnit}`
              : t('notReservable')}
          </p>
          <p>
            <span className='font-semibold'>{t('location')}:</span>{' '}
            {equipment.location} {locationDetails && `(${locationDetails})`}
          </p>
          {requiredQual && (
            <p className='text-xs text-yellow-600 font-semibold'>
              {t('required')}: {requiredQualName}
            </p>
          )}
          {personInCharge && (
            <p className='text-xs text-gray-600'>
              {t('expert')}: {personInCharge.name}
            </p>
          )}
        </div>
        <div className='mt-auto pt-4 space-y-2'>
          <div className='flex space-x-2'>
            {equipment.isReservable && (
              <button
                onClick={() =>
                  openModal({ type: 'scheduleEquipment', props: { equipment } })
                }
                className='flex-1 bg-white hover:bg-gray-100 text-text-primary font-semibold py-2 px-4 rounded-lg border border-gray-300 transition-colors text-sm'
              >
                {t('schedule')}
              </button>
            )}
            <button
              onClick={() =>
                openModal({ type: 'equipmentManuals', props: { equipment } })
              }
              className={`flex-1 font-semibold py-2 px-4 rounded-lg border transition-colors text-sm ${equipment.manualIds && equipment.manualIds.length > 0 ? 'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100' : 'bg-white hover:bg-gray-100 text-text-primary border-gray-300'}`}
            >
              {t('manualsAndInfo')}
            </button>
          </div>
          <div className='flex space-x-2'>
            ...
            <button
              onClick={() =>
                openModal({ type: 'reportIssue', props: { equipment } })
              }
...
const EquipmentTable: React.FC<{
  ...
  const sortedEquipment = useMemo(() => {
    const sortableItems = [...equipmentList];
    if (sortConfig !== null) {
          ...
          {sortedEquipment.length > 0 ? (
            sortedEquipment.map((eq) => (
              <tr key={eq.id}>
                ...
                <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>
                  {eq.isReservable
                    ? `${eq.rate.toLocaleString()} ${isJapanese ? eq.rateUnitJP : eq.rateUnitEN}`
                    : t('notReservable')}
                </td>
                <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-500'>
                  <div className='flex space-x-2'>
                    {(eq.manualIds || []).map((manualId) => {
                      const manual = equipmentManuals.find(
                        (m) => m.id === manualId
                      );
      ...
      <div className='flex justify-between items-center mb-6'>
        ...
        <div>
          <button
            ...
            className={`mr-2 p-2 rounded ${viewMode === 'grid' ? 'bg-ever-blue text-white' : 'bg-gray-200'}`}
          >
            Grid
          </button>
          <button
            onClick={() => setViewMode('table')}
            className={`p-2 rounded ${viewMode === 'table' ? 'bg-ever-blue text-white' : 'bg-gray-200'}`}
          >
            Table
          </button>
        </div>
