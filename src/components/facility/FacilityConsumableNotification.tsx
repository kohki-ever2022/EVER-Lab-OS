...
const FacilityConsumableNotification: React.FC = () => {
  ...
  const { addConsumableNotification, updateConsumableNotificationStatus } =
    useAdminActions();
    ...
    () => [
      {
        ...
        nameJP: '次亜塩素酸',
        nameEN: 'Hypochlorous Acid Solution',
        locations: ['Lab Area A', 'Lab Area B', 'Common Area'],
      },
      {
        id: 'hand-soap',
        nameJP: '手洗い石鹸',
        nameEN: 'Hand Soap',
        locations: ['Washroom 1F', 'Washroom 2F', 'Lab Area A', 'Lab Area B'],
      },
      {
        id: 'hand-paper',
        nameJP: 'ハンドペーパー',
      ...
      {
        id: 'gloves-disposable',
        nameJP: '使い捨て手袋',
        nameEN: 'Disposable Gloves',
        locations: ['Lab Area A', 'Lab Area B'],
      },
    ],
    []
  );
  ...
  const handleSubmitReport = () => {
    ...
    const consumableInfo = facilityConsumables.find(
      (c) => c.id === formData.consumableId
    );
  ...
  const activeNotifications = consumableNotifications.filter(
    (n) => n.status !== 'COMPLETED'
  );
  const completedNotifications = consumableNotifications.filter(
    (n) => n.status === 'COMPLETED'
  );
    ...
    <div className='p-6'>
      <div className='flex justify-between items-center mb-6'>
        <h2 className='text-2xl font-bold'>
          {t('facilityConsumablesShortageNotification')}
        </h2>
        {!isFacilityStaff && (
          <button
            ...
            {formData.consumableId && (
              <div>
                ...
                >
                  <option value=''>{t('select')}</option>
                  {facilityConsumables
                    .find((c) => c.id === formData.consumableId)
                    ?.locations.map((loc) => (
      ...
      <div className='bg-white rounded-lg shadow mb-6'>
        <div className='p-4 border-b bg-gray-50'>
          <h3 className='font-bold'>{t('activeNotifications')}</h3>
        </div>
              ...
              return (
                <div key={notif.id} className='p-4 hover:bg-gray-50'>
                  <div className='flex justify-between items-start mb-2'>
                    <div className='flex-1'>
                      <div className='flex items-center gap-2 mb-1'>
                        ...
                        <span className='font-bold text-lg'>
                          {isJapanese
                            ? notif.consumableNameJP
                            : notif.consumableNameEN}
                        </span>
                        <span
                          className={`px-2 py-1 text-xs font-medium rounded ${priority.color}`}
                        >
                          {t(priority.labelKey)}
                        </span>
                      </div>
                      ...
                      <div className='text-sm text-gray-600 mb-2'>
                        ...
                        {new Date(notif.reportedDate).toLocaleString(
                    ...
                    <div className='ml-4'>
                      <span
                        className={`px-3 py-1 text-sm font-medium rounded ${status.color}`}
                      >
                        {t(status.labelKey)}
                      </span>
                    </div>
      ...
      {completedNotifications.length > 0 && (
        <details className='bg-white rounded-lg shadow'>
          <summary className='p-4 cursor-pointer hover:bg-gray-50 font-bold'>
            {t('completedNotificationsHistory')}{' '}
          ...
          <div className='divide-y border-t'>
            {completedNotifications.map((notif) => (
              <div key={notif.id} className='p-4 text-sm'>
                <div className='flex justify-between'>
                  ...
                  <span className='text-gray-500'>
                    {notif.restockedDate?.toLocaleDateString()}
                  </span>
                </div>
