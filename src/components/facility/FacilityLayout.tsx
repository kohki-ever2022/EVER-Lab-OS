...
const layoutData: BenchGroup[] = [
  ...
  },
  {
    ...
    gridArea: '3 / 8 / 5 / 11',
    items: [
      [{ id: 'E-1' }, { id: 'E-2' }, { id: 'E-3' }],
      [{ id: 'E-4' }, { id: 'E-5' }, { id: 'E-6' }],
    ],
  },
  {
    id: 'group-p1',
...
const Bench: React.FC<BenchProps> = ({ item }) => {
  ...
  const company = assignment?.companyId
    ? companies.find((c) => c.id === assignment.companyId)
    : null;

  const baseClasses =
    'rounded-lg p-1 text-center shadow-sm transition-all duration-200 flex flex-col justify-center items-center w-full h-full transform hover:shadow-xl hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-offset-2';
  const assignedClasses =
    'bg-teal-500 border border-teal-600 text-white focus:ring-teal-400';
  const unassignedClasses =
    'bg-white border-2 border-dashed border-gray-300 text-gray-400 hover:border-teal-400 hover:text-teal-600 focus:ring-teal-300';
  ...
  return (
    <button
      onClick={() =>
        openModal({
          type: 'benchDetails',
          props: { benchInfo: { id: item.id, assignment } },
        })
      }
      className={`${baseClasses} ${company ? assignedClasses : unassignedClasses}`}
    >
      <p
        className={`font-bold text-[clamp(0.75rem,2vw,1.25rem)] leading-tight ${company ? 'text-white' : 'text-slate-700'}`}
      >
        {item.id}
      </p>
      <p
        className={`text-[clamp(0.625rem,1.5vw,0.875rem)] truncate w-full ${company ? 'text-teal-100' : 'text-gray-500'}`}
      >
        {statusText}
      </p>
    </button>
  ...
  return (
    <div>
      <div className='flex flex-col sm:flex-row justify-between items-center mb-6 gap-4'>
        <h2 className='text-3xl font-bold text-lab-blue-dark'>
          {t('facilityLayout')}
        </h2>
      </div>
      <div className='bg-white p-4 sm:p-6 rounded-lg shadow-lg'>
        <div className='overflow-x-auto'>
          <div
            className='relative w-full aspect-[11/10] bg-slate-50 rounded-lg p-4'
            style={{
              minWidth: '600px',
              display: 'grid',
          ...
          >
            ...
            <div
              className='z-10 text-slate-400 font-bold tracking-widest text-[clamp(0.75rem,2vw,1.125rem)]'
              style={{
                gridArea: '2 / 2 / 3 / 5',
                alignSelf: 'end',
                justifySelf: 'center',
