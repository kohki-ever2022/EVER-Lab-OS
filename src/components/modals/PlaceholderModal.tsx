  ...
  return (
    <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 modal-backdrop'>
      <div className='bg-white rounded-lg shadow-xl p-6 w-full max-w-md modal-content'>
        ...
        <p className='text-gray-600 mb-6'>{t('underDevelopment')}</p>
...
export const BenchDetailsModal: React.FC<BenchDetailsModalProps> = ({
  benchInfo,
  onClose,
}) => {
  ...
  const company = benchInfo.assignment?.companyId
    ? companies.find((c) => c.id === benchInfo.assignment.companyId)
    : null;
    ...
    openModal({
      ...
      props: {
        ...
        onConfirm: () => {
          // Placeholder for actual logic
          showToast(
            `Assignment for ${benchInfo.id} cleared. (Demo)`,
            'success'
          );
          onClose();
        },
    ...
    <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 modal-backdrop'>
      <div className='bg-white rounded-lg shadow-xl p-6 w-full max-w-md modal-content'>
        ...
        {company ? (
          <div className='space-y-4'>
            <div>
              ...
              <p className='font-semibold'>
                {isJapanese ? company.nameJP : company.nameEN}
              </p>
            </div>
            <div>
              <p className='text-sm text-gray-500'>{t('startDate')}</p>
              <p className='font-semibold'>
                {new Date(benchInfo.assignment!.startDate).toLocaleDateString()}
              </p>
            </div>
            <button
              onClick={handleClear}
          ...
          <div className='space-y-4'>
            <p className='text-center font-semibold text-green-700 bg-green-50 p-3 rounded-md'>
              {t('available')}
            </p>
            <div>
              ...
              <select
                value={assignToCompanyId}
                onChange={(e) => setAssignToCompanyId(e.target.value)}
