...
const BookEquipmentModal: React.FC<Props> = ({ equipment, onClose }) => {
  ...
  const userProjects = projects.filter(
    (p) => p.companyId === currentUser?.companyId
  );
  ...
  return (
    <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 modal-backdrop'>
      <div className='bg-white rounded-lg shadow-xl p-8 w-full max-w-md modal-content max-h-full overflow-y-auto'>
        <h3 className='text-2xl font-bold mb-6 text-ever-black'>
          {t('bookEquipmentModalTitle')}
        </h3>
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          <div className='border-t pt-4'>
            <div className='flex justify-between items-center'>
              ...
              <span className='text-2xl font-bold text-ever-blue font-mono'>
          ...
          <div className='mt-8 flex justify-end space-x-3'>
            ...
            {bookingError === 'OVERLAP_ERROR' ? (
              ...
              <button
                type='submit'
                className='bg-gradient-to-r from-ever-blue to-ever-purple text-white font-bold py-2 px-4 rounded-full'
              >
                {t('confirmBooking')}
              </button>
            )}
