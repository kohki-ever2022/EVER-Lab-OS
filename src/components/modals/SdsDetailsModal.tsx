...
const SdsDetailsModal: React.FC<Props> = ({ sds, onClose }) => {
  ...
  const [summary, setSummary] = useState<SDSSummary | null>(
    sds.emergencySummary || null
  );
        ...
        <div className='overflow-y-auto pr-2 flex-grow'>
          <div className='bg-blue-50 p-4 rounded-lg mb-4'>
            <h4 className='font-bold text-lg text-blue-800'>
              {t('aiEmergencySummary')}
            </h4>
            {summary ? (
              <div className='mt-2 space-y-3'>
                <SummarySection
                  title={t('sdsHazards')}
                  content={summary.hazards}
                />
                <SummarySection
                  title={t('sdsHandling')}
                  content={summary.handling}
                />
                <SummarySection
                  title={t('sdsStorage')}
                  content={summary.storage}
                />
                <SummarySection
                  title={t('sdsFirstAid')}
                  content={summary.firstAid}
                />
                <SummarySection
                  title={t('sdsSpillResponse')}
                  content={summary.spillResponse}
                />
                <SummarySection
                  title={t('sdsDisposal')}
                  content={summary.disposal}
                />
              ...
              <div className='text-center py-4'>
                ...
                <button
                  ...
                  className='bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg text-sm disabled:bg-gray-400'
                >
          ...
          <div className='mt-4'>
            <a
              ...
              className='inline-block bg-gray-200 text-gray-800 font-semibold py-2 px-4 rounded-lg hover:bg-gray-300'
            >
              {t('openFullSds')}
            </a>
          </div>
