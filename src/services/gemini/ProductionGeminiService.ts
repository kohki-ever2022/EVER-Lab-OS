...
export class ProductionGeminiService implements IGeminiService {
  ...
  constructor() {
    ...
    if (apiKey) {
      ...
      logger.warn(
        'Gemini API key not configured. Gemini features will be disabled.'
      );
    }
  }
  ...
  private isAvailable(language: Language): {
    available: boolean;
    message: string;
    jsonMessage: string;
  } {
  ...
  async getUsageInsights(
    ...
    const usageStrings = recentUsage
      .map((u) => {
    ...
    try {
      const response: GenerateContentResponse = await this.retryWithBackoff(
        () =>
      ...
      logger.error('Failed to fetch usage insights from Gemini after retries', {
  ...
  async getUiUxAnalysis(
    ...
    try {
            ...
            config: {
              ...
              responseSchema: {
                ...
                properties: {
                  overallScore: {
                    type: Type.NUMBER,
                    description: 'Overall UI/UX score (0-100)',
                  },
                  summary: {
                    type: Type.STRING,
                    description: 'A concise summary of the UI/UX quality.',
                  },
                  issues: {
                    type: Type.ARRAY,
                    items: {
      ...
      logger.error('Failed to fetch UI/UX analysis from Gemini after retries', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async summarizeSDSDocument(
    ...
    try {
      ...
      logger.error('Failed to fetch SDS summary from Gemini after retries', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async generateText(prompt: string, language: Language): Promise<string> {
    ...
    try {
      ...
      logger.error('Failed to generate text from Gemini after retries', {
        error: error instanceof Error ? error.message : String(error),
      });
