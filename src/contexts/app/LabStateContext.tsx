...
export interface LabStateContextValue {
  ...
  setConsumableNotifications: React.Dispatch<
    React.SetStateAction<ConsumableNotification[]>
  >;
  co2IncubatorTrackingData: CO2IncubatorTracking[];
  setCo2IncubatorTrackingData: React.Dispatch<
    React.SetStateAction<CO2IncubatorTracking[]>
  >;
  memos: Memo[];
...
export const LabStateProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  const initialData = getMockData();
  const [consumableNotifications, setConsumableNotifications] = useState<
    ConsumableNotification[]
  >(initialData.consumableNotifications);
  const [co2IncubatorTrackingData, setCo2IncubatorTrackingData] = useState<
    CO2IncubatorTracking[]
  >(initialData.co2IncubatorTrackingData);
  const [memos] = useState<Memo[]>(initialData.memos);
  const [waitlist, setWaitlist] = useState<WaitlistEntry[]>(
    initialData.waitlist
  );
