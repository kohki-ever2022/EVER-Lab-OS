  ...
  static getAdapter(): IDataAdapter {
    if (!this.instance) {
      const useMock = (() => {
        try {
          // @ts-ignore
          const envValue = process.env?.VITE_USE_MOCK_DATA;
          if (envValue === undefined || envValue === null || envValue === '') {
            console.warn('⚠️ VITE_USE_MOCK_DATA not set, defaulting to mock mode');
            return true;
          }
          return envValue === 'true';
        } catch (e) {
          console.warn('⚠️ Could not access process.env, defaulting to mock mode');
          return true;
        }
      })();
